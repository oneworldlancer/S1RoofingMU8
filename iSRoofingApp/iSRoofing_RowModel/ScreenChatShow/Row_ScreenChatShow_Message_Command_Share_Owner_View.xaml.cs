...
namespace S1RoofingMU.iSRoofingApp.iSRoofing_RowModel.ScreenChatShow
{
    ...
    public partial class Row_ScreenChatShow_Message_Command_Share_Owner_View : ContentView
    {
        public Row_ScreenChatShow_Message_Command_Share_Owner_View()
        {
            InitializeComponent();
    ...
    if (iGroupModel != null)
      ...
      SRoofingEnum_MessageCenter.MessageCenter_CHAT_NEW_SCREENCHATSHOW_MESSAGE , iBroadcastModel );
        ...
        async void imgBtn_Share_Clicked ( object sender , EventArgs e )
        {
            ...
            // await   (( Page_Chat_Dashboard ) Parent.BindingContext)
            //        .Picker_ShareMedia ( iGroupModel);
            }
            catch ( Exception ex )
            {
                SRoofing_DebugManager.Debug_ShowSystemMessage ( ex.Message.ToString ( ) );
